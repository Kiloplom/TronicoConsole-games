...
void InitializeGameVariables() {
    ...
    ball = (Ball){PONG_BOARD_WIDTH / 2 + 10, PONG_BOARD_HEIGHT / 2 + 5, RIGHT_DOWN};
    game_on_flag_1 = TRUE;
}
...
void UpdateFrame1() {
	...
	for(i=0;game_on_flag_1;i++) {
		Sleep(16);
		if(_kbhit()) {				// FPS (frames per second):  1000 (milliseconds in second) / 16 (time interval between frames in milliseconds) = 62.5
			char key = getch();	
			switch(key) {
